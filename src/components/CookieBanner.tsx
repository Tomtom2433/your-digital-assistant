...
function loadSaved(): { visible: boolean; prefs: Prefs } {
  ...
  try {
    ...
    if (raw === "accepted") return { visible: false, prefs: { necessary: true, functional: true,  analytics: false } };
    if (raw === "refused")  return { visible: false, prefs: { necessary: true, functional: false, analytics: false } };
...
function Toggle({ checked, onChange, disabled = false }: {
  ...
  const bg = disabled
    ...
    : checked
    ? "linear-gradient(90deg, #C5A374, #E8D4A0)"
  ...
  return (
    <button
      ...
      style={{
        ...
        border: "none", background: bg, position: "relative",
        cursor: disabled ? "not-allowed" : "pointer",
        transition: "background 0.3s ease", outline: "none", padding: 0,
      }}
    >
      <span style={{
...
const CATEGORIES: CatDef[] = [
  {
    ...
    mandatory: true,
    cookies: [
      { name: "meliya-cookie-consent",      purpose: "Mémorise votre choix de consentement",  duration: "13 mois" },
      { name: "meliya-cookie-consent-date", purpose: "Horodatage du consentement",             duration: "13 mois" },
    ],
  },
  {
    id: "functional",
    ...
    mandatory: false,
    cookies: [
      { name: "meliya-theme", purpose: "Mémorise votre thème choisi (clair / sombre)", duration: "Local" },
    ],
  },
  {
    id: "analytics",
    ...
    description: "Permettent de mesurer l'audience du site. Aucun cookie analytique n'est utilisé actuellement.",
    mandatory: false,
    cookies: [],
  },
];
...
export function CookieBanner() {
  ...
  const [prefs,    setPrefs]    = useState<Prefs>({ necessary: true, functional: true, analytics: false });
  ...
  const acceptAll  = () => { saveConsent({ necessary: true, functional: true,  analytics: false }); setVisible(false); };
  const refuseAll  = () => { saveConsent({ necessary: true, functional: false, analytics: false }); setVisible(false); };
  ...
  const ink       = isDark ? "rgba(245,240,250,0.92)" : "#8C6A43";
  const inkMuted  = isDark ? "rgba(245,240,250,0.52)" : "rgba(140,106,67,0.55)";
  const catBg     = isDark ? "rgba(255,255,255,0.04)" : "rgba(197,163,116,0.04)";
  const divider   = isDark ? "rgba(200,160,82,0.14)"  : "rgba(200,160,82,0.18)";

  /* ── Bouton helper ── */
  const btnBase: React.CSSProperties = {
    padding: "9px 18px", borderRadius: "100px",
    fontFamily: "var(--font-display)", fontSize: "10.5px",
  ...
  return (
    <div
      ...
      style={{
        ...
        margin: "0 auto", maxWidth: "860px", zIndex: 200,
        background: bg,
        ...
        boxShadow: isDark
          ...
          : "0 8px 48px rgba(197,163,116,0.12), 0 2px 8px rgba(197,163,116,0.07)",
      }}
    >
      {/* ═══ VUE RAPIDE ═══ */}
      <div style={{ padding: "18px 22px 16px", display: "flex", alignItems: "flex-start", gap: "16px", flexWrap: "wrap" }}>

        {/* Texte */}
        <div style={{ flex: 1, minWidth: "220px" }}>
          <p style={{
            fontFamily: "var(--font-display)", fontSize: "13px", fontWeight: 700,
            letterSpacing: "0.06em", color: ink, margin: "0 0 5px",
          }}>
            🍪 Vos préférences de cookies
          </p>
          <p style={{ fontFamily: "var(--font-serif)", fontSize: "13px", color: inkMuted, margin: 0, lineHeight: 1.65 }}>
            Ce site utilise des cookies nécessaires à son fonctionnement et des cookies fonctionnels
            pour mémoriser vos préférences.{" "}
            <Link
              to="/mentions-legales"
              hash="cookies"
              style={{ color: "#C5A374", textDecoration: "underline", textUnderlineOffset: "3px" }}
            >
        ...
        <div style={{ display: "flex", gap: "8px", alignItems: "center", flexWrap: "wrap", flexShrink: 0 }}>
          <button type="button" onClick={refuseAll} style={{
            ...btnBase,
          ...
          <button type="button" onClick={acceptAll} style={{
            ...btnBase,
            border: "1px solid rgba(184,146,42,0.42)",
            background: "linear-gradient(135deg, #C8A03C 0%, #E4C878 50%, #FDF5E0 100%)",
            color: "#1A0E06",
      ...
      {expanded && (
        <div style={{ borderTop: `1px solid ${divider}`, padding: "16px 22px 20px" }}>
            ...
            {CATEGORIES.map((cat) => {
              ...
              return (
                <div key={cat.id} style={{
                  background: catBg, border: `1px solid ${divider}`,
                  ...
                  >
                    <span style={{ color: "#C5A374", display: "flex", alignItems: "center", flexShrink: 0 }}>
                      {cat.icon}
                    </span>
                    <span style={{
                      flex: 1, fontFamily: "var(--font-display)", fontSize: "11.5px",
                      fontWeight: 700, letterSpacing: "0.06em", color: ink,
                    }}>
                      {cat.label}
                    </span>
                    {cat.mandatory && (
                      <span style={{
                        fontSize: "9.5px", color: "#C5A374", fontFamily: "var(--font-display)",
                        letterSpacing: "0.08em", textTransform: "uppercase", opacity: 0.8,
                        border: "0.5px solid rgba(200,160,82,0.35)", borderRadius: "100px",
                        padding: "2px 7px",
                      }}>
                        Requis
                      </span>
                    ...
                    />
                    <span style={{ color: inkMuted, display: "flex", alignItems: "center", marginLeft: "4px", flexShrink: 0 }}>
                  ...
                  {isOpen && (
                    <div style={{ padding: "0 14px 13px", borderTop: `1px solid ${divider}` }}>
                      <p style={{
                        fontFamily: "var(--font-serif)", fontSize: "12.5px",
                        color: inkMuted, margin: "10px 0 10px", lineHeight: 1.6,
                        fontStyle: "italic",
                      }}>
                        {cat.description}
                      </p>
                      ...
                      {cat.cookies.length > 0 ? (
                        <div style={{ overflowX: "auto" }}>
                          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "11.5px" }}>
                            <thead>
                              <tr>
                                {["Nom du cookie", "Finalité", "Durée"].map((h) => (
                                  <th key={h} style={{
                                    textAlign: "left", fontFamily: "var(--font-display)", fontWeight: 600,
                                    letterSpacing: "0.06em", color: "#C5A374", fontSize: "9.5px",
                                    textTransform: "uppercase", padding: "3px 8px 7px 0",
                            ...
                            <tbody>
                              {cat.cookies.map((c) => (
                                <tr key={c.name}>
                                  <td style={{ padding: "6px 8px 6px 0", color: ink, fontFamily: "monospace", fontSize: "11px", whiteSpace: "nowrap" }}>{c.name}</td>
                                  <td style={{ padding: "6px 8px 6px 0", color: inkMuted, fontFamily: "var(--font-serif)", lineHeight: 1.4 }}>{c.purpose}</td>
                                  <td style={{ padding: "6px 0", color: inkMuted, fontFamily: "var(--font-serif)", whiteSpace: "nowrap" }}>{c.duration}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                        ...
                        <p style={{ fontFamily: "var(--font-serif)", fontSize: "12px", color: inkMuted, fontStyle: "italic", margin: 0 }}>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end" }}>
            <button type="button" onClick={saveCustom} style={{
              ...btnBase,
              border: "1px solid rgba(184,146,42,0.42)",
              background: "linear-gradient(135deg, #C8A03C 0%, #E4C878 50%, #FDF5E0 100%)",
              color: "#1A0E06",
